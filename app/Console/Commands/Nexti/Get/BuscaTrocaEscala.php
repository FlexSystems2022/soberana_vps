...
use App\Action\ScheduleTransfer\GetLastUpdateScheduleTransfer;

class BuscaTrocaEscala extends CommandNexti
{
    /**
     ...
     * @var string
     */
    protected $signature = 'BuscaTrocaEscala';
    ...
    public function handle(): int
    {
        ...
        $action = new GetLastUpdateScheduleTransfer();
     ...
     *
     * @param \App\Action\ScheduleTransfer\GetLastUpdateScheduleTransfer $action
     * @param \Illuminate\Support\Carbon $start
     * @param \Illuminate\Support\Carbon $finish
     * @return void
     **/
    private function getData(GetLastUpdateScheduleTransfer $action, Carbon $start, Carbon $finish): void
    {
        $page = 0;
        $total_page = 1;
    ...
    protected function createOrUpdate(array $troca): void
    {
        $data = [
            ...
            'ROTATIONCODE' => $troca['rotationCode'] ?? null,
            'TRANSFERDATETIME' => $this->ajusteData($troca['transferDateTime'] ?? null, 'Y-m-d H:i:s'),
            'SCHEDULEEXTERNALID' => $troca['scheduleExternalId'] ?? null,
            'SCHEDULEID' => $troca['scheduleId'] ?? null,
            'ROTATIONID' => $troca['rotationId'] ?? null,
            'REMOVED' => $troca['removed'] ?? false,
            'USERREGISTERID' => $troca['userRegisterId'] ?? null
        ];

        $model = DB::table('NEXTI_RET_TROCA_ESCALA')->where('ID', $data['ID']);

        $found = $model->first();
        if(!$found) {
            DB::table('NEXTI_RET_TROCA_ESCALA')->insert($data);
