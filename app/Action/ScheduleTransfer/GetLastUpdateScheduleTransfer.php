...
class GetLastUpdateScheduleTransfer extends NextiAction
{
	/**
	 * Action Get Schedule Transfer
	 * 
	...
	public function get(RestClient $client, array $fields): array
	{
		try {
	        ...
	        $request = $client->get("scheduletransfers/lastupdate/start/{$payload['start']}/finish/{$payload['finish']}",
	        	query: [
		            'page' => $payload['page'],
		            'size' => $payload['size'],
	    	    ]
