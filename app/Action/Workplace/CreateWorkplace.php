...
class CreateWorkplace extends NextiAction
{
	/**
	 * Action Create Workplace
	 * 
	 * @param \App\Shared\Provider\RestClient $client
	 * @param \App\Models\Workplace $posto
	 * @return array
	 **/
	public function create(RestClient $client, Workplace $posto): array
	{
		try {
	        $payload = $this->makePayload($posto);

	        $request = $client->post('workplaces', options: [
	            'json' => $payload
	        ]);
