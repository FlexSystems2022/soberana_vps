...
class CreatePeople extends NextiAction
{
	/**
	 ...
	 * @return array
	 **/
	public function create(RestClient $client, People $colaborador): array
	{
		try {
	        $payload = $this->makePayload($colaborador);

	        $request = $client->post('persons', options: [
	            'json' => $payload
	        ]);
	...
	private function makePayload(People $colaborador): array
	{
        ...
        if(!$payload['careerId'] && !$payload['externalCareerId']) {
        	throw new \Exception('Colaborador não possui cargo vinculado!');
        }
